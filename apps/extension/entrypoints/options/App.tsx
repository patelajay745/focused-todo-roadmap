          ...
          {roadmaps.length === 0 ? (
            ...
            <ul className="divide-y divide-white/5">
              {roadmaps.map((roadmap) => {
                ...
                return (
                  <li key={roadmap.id} className="flex items-center gap-3 py-3 first:pt-0">
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-sm font-medium">{roadmap.title}</p>
                      <p className="mt-0.5 text-xs text-ink-muted">
                        {done}/{roadmap.tasks.length} done ·{' '}
                        {formatDuration(totalSec(roadmap.tasks))} ·{' '}
                        {roadmap.schedule.length} days · ends {roadmap.schedule.at(-1)?.date}
                      </p>
                    </div>
                    <Button variant="danger" onClick={() => void remove(roadmap.id)}>
                      Delete
                    </Button>
                  </li>
                );
